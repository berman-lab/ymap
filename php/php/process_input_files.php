<?php
function process_input_files($ext,$name,$projectPath,$key,$user,$project,$directory,$output, $condensedLogOutput,$logOutput) {
...
fwrite($logOutput, "\t| projectPath = ".$projectPath."\n");
	...
	if ((strcmp($ext_new,"fastq") == 0) || (strcmp($ext_new,"fq") == 0)) {
		...
		if (strcmp($ext_new,"bam") == 0) {
			...
			$null = shell_exec("sh /heap/hapmap/bermanlab/sh/bam2sam.sh ".$user." ".$project." ".$directory." ".$name_new);
		...
		$currentDir = getcwd();
		chdir($projectPath);
		$null       = shell_exec("sh /heap/hapmap/bermanlab/sh/sam2fastq.sh ".$user." ".$project." ".$directory." ".$name_new);
		chdir($currentDir);
		...
		$currentDir = getcwd();
		chdir($projectPath);
		$null       = shell_exec("sh /heap/hapmap/bermanlab/sh/Gareth2pileups.sh ".$user." ".$project." ".$directory." ".$name_new);
		chdir($currentDir);
		// sam2fastq.sh user project main_dir inputFile;
		fwrite($output, "null1\n");
		fwrite($output, "null2\n");
