...
fprintf(['\nGenerating ChARM figures from project "' projectName '" SNP ratio data.\n']);
...
%%=========================================================================
% Load Common_SNP_ratio file for project : 'chr_SNPdata_ratios'.
%--------------------------------------------------------------------------
% function [SNP_count, Ratio_sum] = Collect_SNP_ratios(chr_size, probeset1);
% [chr_SNPdata_ratios{chr,1}, chr_SNPdata_ratios{chr,2}] = Collect_SNP_ratios(chr_size, probeset1);   : Common_SNPratio.mat
% 
dataFile = [workingDir 'Common_SNPratio.mat'];
fprintf(['\nLoading Common_SNPratio file for "' projectName '" : ' dataFile '\n']);
load(dataFile);
vars          = who('-file',dataFile);
for chr = 1:8
	%%%% Calculate SNP ratio average per bin.
	SNP_ratio_data{chr} = zeros(1,ceil(chr_size(chr)/bases_per_bin));
	fprintf(['chr' num2str(chr) ' length = ' num2str(length(SNP_ratio_data{chr})) '\n']);
	for i = 1:length(chr_SNPdata_ratios{chr,1})
		if (chr_SNPdata_ratios{chr,1}(i) > 0)
			SNP_ratio_average = chr_SNPdata_ratios{chr,2}(i)/chr_SNPdata_ratios{chr,1}(i);
		else
			SNP_ratio_average = 0;
		end;
		SNP_ratio_data{chr}(i) = SNP_ratio_average;
	end;

	%%%% Fill in gaps in SNP ratio averages caused by low SNP-density regions.
	% 1. Scan from left to right in each chromosome, looking for 0-counts.
	% 2. Once a 0-count is found, scan to right to find end of 0-count-run.
	% 3. Once a 0-count-run is defined, fill in the edge values.
	%    a. If run is even-length, fill in evenly.
	%    b. If run is odd-length, fill in evenly and midpoint is average of ends.
	for i = 1:length(chr_SNPdata_ratios{chr,1})
		if (chr_SNPdata_ratios{chr,1}(i) == 0)
			run_start = i;
			run_end   = i;
			j         = 1;
			if (i+j < length(chr_SNPdata_ratios{chr,1}))
				test_val = chr_SNPdata_ratios{chr,1}(i+j);
				while (test_val == 0)
					run_end = i+j;
					j       = j+1;
					if (i+j < length(chr_SNPdata_ratios{chr,1}))
						...
						run_end  = length(chr_SNPdata_ratios{chr,1});
						test_val = 1;
					end;
				end;
			end;
%			while (i+j < length(chr_SNPdata_ratios{chr,1}))
%				if (chr_SNPdata_ratios{chr,1}(i+j) == 0)
%					run_end = i+j;
%					j       = j+1;
%				end;
%			end;
			...
			if (run_start == 1) && (run_end == length(chr_SNPdata_ratios{chr,1})) %%%% ---------------------------- There is no data for this chromosome.
				value_start = 0;                                  value_end   = 0;
			elseif (run_start == 1) %%%% -------------------------------------------------------------------------- The start of the chromosome is empty.
				value_start = SNP_ratio_data{chr}(run_end+1);     value_end   = SNP_ratio_data{chr}(run_end+1);
			elseif (run_end == length(chr_SNPdata_ratios{chr,1})) %%%% -------------------------------------------- The end of the chromosome is empty.
				value_start = SNP_ratio_data{chr}(run_start-1);   value_end   = SNP_ratio_data{chr}(run_start-1);
			else %%%% --------------------------------------------------------------------------------------------- A section inside the chromosome is empty.
				value_start = SNP_ratio_data{chr}(run_start-1);   value_end   = SNP_ratio_data{chr}(run_end+1);
			end;
			if (mod(run_length,2) == 0) % odd number
				run_offset = 0;
				%%%% run edge bins, incrementing inwards.
				while (run_length > 1)
					SNP_ratio_data{chr}(run_start+run_offset) = value_start;
					SNP_ratio_data{chr}(run_end  +run_offset) = value_end;
					run_length = run_length-2;
					run_offset = run_offset+1;
				end;
				%%%% Center bin is filled in with the average of each end value.
				% SNP_ratio_data{chr}(run_start) = (value_start + value_end)/2;
				% SNP_ratio_data{chr}(run_start) = max(value_start,value_end);
			else % even number
				run_offset = 0;
				%%%% run edge bins, incrementing inwards.
				while (run_length > 0)
...
for chr = 1:8
	examined_data{chr} = SNP_ratio_data{chr}*2;
end;
...
if (temp_figures == true)
	...
	saveas(fig,[workingDir 'ChARM_test.1.snp.eps'], 'epsc');
	saveas(fig,[workingDir 'ChARM_test.1.snp.png'], 'png');
	delete(fig);
	...
	saveas(fig,[workingDir 'ChARM_test.2.snp.eps'], 'epsc');
	saveas(fig,[workingDir 'ChARM_test.2.snp.png'], 'png');
	delete(fig);
	...
	saveas(fig,[workingDir 'ChARM_test.3.snp.eps'], 'epsc');
	saveas(fig,[workingDir 'ChARM_test.3.snp.png'], 'png');
	delete(fig);
	...
	saveas(fig,[workingDir 'ChARM_test.4.snp.eps'], 'epsc');
	saveas(fig,[workingDir 'ChARM_test.4.snp.png'], 'png');
	delete(fig)
end;
...
%.....................................................................................................
% The goal of this section is to perform local optimization of edge placement.
%-----------------------------------------------------------------------------------------------------
...
%.....................................................................................................
% 1. Delete edges when side window distrubutions are too similar.
%       ChARM used median of data in each side window.
% 2. Convert pairs of edges that are too close into single edges.
% 3. Force left and right edges to left and right of datasets.
%-----------------------------------------------------------------------------------------------------
	...
	for t = 1:num_starting_edges
		...
		if (num_edges > 2)
			...
			for edge = 2:(num_edges-1)
				...
				fprintf(['&&&& edge' num2str(edge) ' = [mL/mR/dm = ' num2str(med_j1) '/' num2str(med_j2) '/' num2str(delta_median) '; signal = ' num2str(signal(edge)) ']']);
			end;
		...
		%----------------------------------------------------------------------
		% Re-sort and save edge positions.
		%----------------------------------------------------------------------
...
%			set(gca,'YTickLabel',{'','',''});
%		    end;
%		    saveas(fig,[workingDir 'ChARM_test.5.snp.' num2str(chr_fig) '_' num2str(t,'%02d') '.eps'], 'epsc');
%		    delete(fig)
%		end;
    end;
end;
...
if (temp_figures == true)
	...
	for chr = 1:num_chr
		...
		for edge = 1:length(locs{chr})
			plot([locs{chr}(edge) locs{chr}(edge)], [0 maxY],'color',[0 1 0]);
			var1 = chr;
			var2 = locs{chr}(edge);
			fprintf(['c2.breakpoint = chr' num2str(var1) ':' num2str(var2) '\n']);
		end;
		hold off;
		xlim([0,chr_size(chr)*chr_length_scale_multiplier]);    ylim([0,maxY]);
		set(gca,'YTick',[0 maxY/2 maxY]);    set(gca,'YTickLabel',{'','',''});
	end;
	saveas(fig,[workingDir 'ChARM_test.5.snp.eps'], 'epsc');
	saveas(fig,[workingDir 'ChARM_test.5.snp.png'], 'png');
	delete(fig)
end;
...
fprintf(['\nSaving Common_ChARM_SNPrations file for "' projectName '" : ' dataFile '\n']);
...
for chr = 1:num_chr
	...
	for edge = 1:num_edges
		if (position(edge) == 1) || (position(edge) == chr_size)
			...
			fprintf(['b2.breakpoint = chr' num2str(var1) ':' num2str(var2) '\n']);
		end;
	end;
end;
