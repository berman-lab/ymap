...
%%================================================================================================
% Load pre-processed RNAseq fragment CNV data for project.
%-------------------------------------------------------------------------------------------------
if (exist([main_dir 'users/' user '/projects/' project '/fragment_CNV_data.mat'],'file') == 0)
    ...
    datafile_RNAseq  = [main_dir 'users/' user '/projects/' project '/preprocessed_CNVs.RNAseq.txt'];
    data_RNAseq      = fopen(datafile_RNAseq);
    count            = 0;
    fragments_CNV    = [];
    while ~feof(data_RNAseq)
		% Load fragment data from pre-processed text file, single line.
		dataLine = fgetl(data_RNAseq);
...
%%================================================================================================
% Load pre-processed RNAseq fragment CNV data for parent project.
%-------------------------------------------------------------------------------------------------
if (strcmp(project,parent) == 1)
...
%%================================================================================================
% Load pre-processed RNAseq fragment GC-bias data for genome.
%-------------------------------------------------------------------------------------------------
fprintf(['RNAseq_bins_GC_ratios_file :\n\t' main_dir 'users/' genomeUser '/genomes/' genome '/' FastaName '.GC_ratios.MfeI_MboI.txt\n']);
...
%%================================================================================================
% Load pre-processed RNAseq fragment repetitiveness data for genome.
%-------------------------------------------------------------------------------------------------
fprintf(['RNAseq_bins_repetitiveness_file :\n\t' main_dir 'users/' genomeUser '/genomes/' genome '/' FastaName '.repetitiveness.MfeI_MboI.txt\n']);
...
for fragID = 1:numFragments
	...
	if (fragment_data(fragID).ave_reads == 0)
		fragment_data(fragID).usable = 0;
		Y_reads_project(fragID) = fragment_data(fragID).ave_reads;
	else
		Y_reads_project(fragID) = log2(fragment_data(fragID).ave_reads);
	end;
	...
	if (fragment_data(fragID).ave_reads_parent == 0)
		...
		Y_reads_parent(fragID)  = log2(fragment_data(fragID).ave_reads_parent);
	end;
end;
...
Y_reads_project_corrected_1 = (Y_reads_project+10)./(Y_fitCurve1_project+10)*Y_target;
%-------------------------------------------------------------------------------------------------
...
hold off;
axis normal;
h = title('Examine bias between RNAseq fragment length and read count. [EXP]');   set(h, 'FontSize', 10);
...
if (useParent)
	...
	Y_reads_parent_corrected_1 = (Y_reads_parent+10)./(Y_fitCurve1_parent+10)*Y_target;
	%-------------------------------------------------------------------------------------------------
	...
	hold off;
	axis normal;
	h = title('Examine bias between RNAseq fragment length and read count. [REF]');   set(h, 'FontSize', 10);
	h = ylabel('Average Reads');     set(h, 'FontSize', 10);
	h = xlabel('Fragment Length');   set(h, 'FontSize', 10);
	set(gca,'FontSize',10);
	xlim([0 10000]);
...
%		if (X_length(fragID)                    >  1000);   fragment_data(fragID).usable = 0;   end;
	...
	hold off;
	axis normal;
	h = title('Corrected Read count vs. RNAseq fragment length. [EXPERIMENT]');   set(h, 'FontSize', 10);
...
%			if (X_length(fragID)                   >  1000);   fragment_data(fragID).usable_parent = 0;   end;
		...
		hold off;
		axis normal;
		h = title('Corrected Read count vs. RNAseq fragment length. [REFERENCE]');   set(h, 'FontSize', 10);
		h = ylabel('Corrected Reads');   set(h, 'FontSize', 10);
		h = xlabel('Fragment Length');   set(h, 'FontSize', 10);
		set(gca,'FontSize',10);
%		ylim([0 4]);
...
%%================================================================================================
% Prepare for LOWESS fitting 2 : correcting GC_bias.
%-------------------------------------------------------------------------------------------------
if (performGCbiasCorrection)
	...
	%-------------------------------------------------------------------------------------------------
	% LOWESS fitting project 2 : correcting GC_bias.
	%-------------------------------------------------------------------------------------------------
	% Perform LOWESS fitting.
	...
	%-------------------------------------------------------------------------------------------------
	fprintf('Subplot 5/15 : [EXP] (Corrected reads 1) vs. (Fragment GC ratio).\n');
	sh(5) = subplot(3,4,9);
	hold on;
	for fragID = 1:numFragments
		if (fragment_data(fragID).usable == 1)
	...
	if (useParent)
		%-------------------------------------------------------------------------------------------------
		% LOWESS fitting parent 2 : correcting GC_bias.
		%-------------------------------------------------------------------------------------------------
		% Perform LOWESS fitting.
		...
		%-------------------------------------------------------------------------------------------------
		fprintf('Subplot 6/15 : [REF] (Corrected reads 1) vs. (Fragment GC ratio).\n');
		sh(6) = subplot(3,4,11);
		hold on;
		for fragID = 1:numFragments
			if (fragment_data(fragID).usable == 1)
	...
	%-------------------------------------------------------------------------------------------------
	fprintf('Subplot 7/15 : [EXP] (Corrected reads 2) vs. (Fragment GC ratio).\n');
	sh(7) = subplot(3,4,10);
	hold on;
	for fragID = 1:numFragments
		% Add corrected ave_data to fragment data structure.
	...
	if (useParent)
		...
		%-------------------------------------------------------------------------------------------------
		fprintf('Subplot 8/15 : [REF] (Corrected reads 2) vs. (Fragment GC ratio).\n');
		sh(8) = subplot(3,4,12);
		hold on;
		for fragID = 1:numFragments
			% Add corrected ave_data to fragment data structure.
...
set(fig1,'PaperPosition',[0 0 8 3]*4);
...
%%================================================================================================
% Prepare for LOWESS fitting 2 : correcting repetitiveness bias.
%-------------------------------------------------------------------------------------------------
if (performRepetbiasCorrection)
	fprintf('\tPreparing for LOWESS fitting 3 : repetitiveness vs. corrected_count_2.\n');
	...
	if (length(X_repet_project_trimmed) > 0)
		[newX3_project, newY3_project] = optimize_mylowess(X_repet_project_trimmed,Y_reads_project_trimmed,10, 0, 3);
	else
		newX3_project = [];
		newY3_project = [];
	end;
	...
	Y_target                    = 1;
	Y_fitCurve3_project         = interp1(newX3_project,newY3_project,X_repet_project,'spline');
	...
	for fragID = 1:numFragments
		if (fragment_data(fragID).usable == 1)
			if (fragment_data(fragID).ave_reads_corrected_2 < Y_fitCurve3_project(fragID))
				plot(fragment_data(fragID).repet,fragment_data(fragID).ave_reads_corrected_2,'.', 'color', [0.0, 0.66667, 0.33333], 'MarkerSize',1);
			else
				plot(fragment_data(fragID).repet,fragment_data(fragID).ave_reads_corrected_2,'.', 'color', [0.0, 0.45, 0.55], 'MarkerSize',1);
			end;
		end;
	end;
	...
	hold off;
	axis normal;
	h = title('Repetitiveness vs. Corrected reads 2. [EXP]');   set(h, 'FontSize', 10);
	h = ylabel('Corrected reads 2');  set(h, 'FontSize', 10);
	...
	if (useParent)
		...
		% Perform LOWESS fitting.
		fprintf('\tLOWESS fitting to parent data.\n');
		[newX3_parent, newY3_parent] = optimize_mylowess(X_repet_parent_trimmed,Y_reads_parent_trimmed,10, 0, 3);
		fprintf('\tLOWESS fitting to parent data complete.\n');
		% Calculate length_bia_corrected GC_bias_corrected repetitiveness_bias_corrected ave_read_count data for plotting and later analysis.
		Y_target                   = 1;
		Y_fitCurve3_parent         = interp1(newX3_parent,newY3_parent,X_repet_parent,'spline');
		Y_reads_parent_corrected_3 = Y_reads_parent./Y_fitCurve3_parent*Y_target;
		%-------------------------------------------------------------------------------------------------
		% Show repetitiveness bias vs. corrected_2 read depth for parent.
		%-------------------------------------------------------------------------------------------------
		fprintf('Subplot 10/15 : [REF] (Corrected reads 2) vs. (Fragment repetitiveness).\n');
		sh(10) = subplot(3,4,3);
		hold on;
		for fragID = 1:numFragments
			if (fragment_data(fragID).usable == 1)
				if (fragment_data(fragID).ave_reads_parent_corrected_2 < Y_fitCurve3_parent(fragID))
					plot(fragment_data(fragID).repet,fragment_data(fragID).ave_reads_parent_corrected_2,'.', 'color', [0.0, 0.66667, 0.33333], 'MarkerSize',1);
				else
					plot(fragment_data(fragID).repet,fragment_data(fragID).ave_reads_parent_corrected_2,'.', 'color', [0.0, 0.45, 0.55], 'MarkerSize',1);
				end;
			end;
		end;
		...
		hold off;
		axis normal;
		h = title('Repetitiveness vs. Corrected reads 2. [REF]');   set(h, 'FontSize', 10);
		h = ylabel('Corrected reads 1');  set(h, 'FontSize', 10);
		h = xlabel('Repetitiveness');     set(h, 'FontSize', 10);
		set(gca,'FontSize',10);
		ylim([0 3]);
	...
	fprintf('\n');
	plot([0 2000],[1 1],'color','k','linestyle','-', 'linewidth',1);
	hold off;
	axis normal;
...
hold off;
axis normal;
h = title('Corrected_3 read count vs. RNAseq fragment length. [EXP]');   set(h, 'FontSize', 10);
h = ylabel('Corrected reads');   set(h, 'FontSize', 10);
h = xlabel('Fragment Length');   set(h, 'FontSize', 10);
set(gca,'FontSize',10);
ylim([0 3]);
...
if (useParent)
	...
	hold off;
	axis normal;
	h = title('Corrected_3 read count vs. RNAseq fragment length. [REF]');   set(h, 'FontSize', 10);
	h = ylabel('Corrected reads');   set(h, 'FontSize', 10);
	h = xlabel('Fragment Length');   set(h, 'FontSize', 10);
	set(gca,'FontSize',10);
	ylim([0 3]);
...
hold off;
axis normal;
h = title('Final Corrected Read count vs. RNAseq fragment length.');   set(h, 'FontSize', 10);
h = ylabel('Corrected reads');   set(h, 'FontSize', 10);
h = xlabel('Fragment Length');   set(h, 'FontSize', 10);
set(gca,'FontSize',10);
ylim([0 3]);
...
%%================================================================================================
% Load 'preprocessed_SNPs.RNAseq.txt' file for CNV_filtering, then output to 'preprocessed_SNPs.RNAseq.CNV_filtered.txt'
%.................................................................................................
% SNP data from CNV filtered fragments is discarded.
%-------------------------------------------------------------------------------------------------
datafile = [projectDir 'preprocessed_SNPs.RNAseq.txt'];
...
%	end;
%end;
datafile = [projectDir 'preprocessed_SNPs.RNAseq.CNV_filtered.txt'];
...
set(fig2,'PaperPosition',[0 0 8 3]*4);
...
%%=========================================================================
%%= Analyze corrected CNV data for RNAseq datasets. =======================
%%=========================================================================
...
for chr = 1:num_chrs   % number of chrs.
	if (chr_in_use(chr) == 1)
		...
		chr_CNVdata_RNAseq{chr,1} = zeros(1,ceil(chr_size(chr)/bases_per_bin));
		chr_CNVdata_RNAseq{chr,2} = zeros(1,ceil(chr_size(chr)/bases_per_bin));
		chr_CNVdata_RNAseq{chr,3} = zeros(1,ceil(chr_size(chr)/bases_per_bin));
		chr_CNVdata_RNAseq{chr,4} = zeros(1,ceil(chr_size(chr)/bases_per_bin));
	end;
end;
% Output chromosome lengths to log file.
for i = 1:length(chr_name)
	fprintf(['\nchr' num2str(i) ' = ''' chr_name{i} '''.\tCGHlength = ' num2str(length(chr_CNVdata_RNAseq{i,1}))]);
end;
...
if (exist([main_dir 'users/' user '/projects/' project '/corrected_CNV.project.mat'],'file') == 0)
	...
	for fragID = 1:numFragments
		if ((fragment_data(fragID).usable == 1) && (fragment_data(fragID).usable_parent == 1))
			...
			% the final per-fragment CNV values.   This should correct the final CNV artifact found in the RNAseq data from AForche
			...
			if (chr > 0) && (count1 > 0) && (count2 > 0)
				...
				if (val1 == val2)
					...
					if (val1 <= length(chr_CNVdata_RNAseq{chr,1}))
						% project standard_bins data.
						% chr_CNVdata_RNAseq{chr,1}(val1) = chr_CNVdata_RNAseq{chr,1}(val1) + count1*fragLength;
						chr_CNVdata_RNAseq{chr,1}(val1)   = chr_CNVdata_RNAseq{chr,1}(val1) + count1*fragLength/finalFrag_median;
						chr_CNVdata_RNAseq{chr,2}(val1)   = chr_CNVdata_RNAseq{chr,2}(val1) + fragLength;
						% parent standard_bins data.
						chr_CNVdata_RNAseq{chr,3}(val1)   = chr_CNVdata_RNAseq{chr,3}(val1) + count2*fragLength;
						chr_CNVdata_RNAseq{chr,4}(val1)   = chr_CNVdata_RNAseq{chr,4}(val1) + fragLength;
					end;
				else % (val1 < val2)
					...
					if (val1 <= length(chr_CNVdata_RNAseq{chr,1}))
						% project standard_bins data.
						% chr_CNVdata_RNAseq{chr,1}(val1) = chr_CNVdata_RNAseq{chr,1}(val1) + count1*fragLength1;
						chr_CNVdata_RNAseq{chr,1}(val1)   = chr_CNVdata_RNAseq{chr,1}(val1) + count1*fragLength1/finalFrag_median;
						chr_CNVdata_RNAseq{chr,2}(val1)   = chr_CNVdata_RNAseq{chr,2}(val1) + fragLength1;
						% parent standard_bins data.
						chr_CNVdata_RNAseq{chr,3}(val1)   = chr_CNVdata_RNAseq{chr,3}(val1) + count2*fragLength1;
						chr_CNVdata_RNAseq{chr,4}(val1)   = chr_CNVdata_RNAseq{chr,4}(val1) + fragLength1;
					end;

					% Add data to second bin.
					if (val2 <= length(chr_CNVdata_RNAseq{chr,1}))
						% project standard_bins data.
						% chr_CNVdata_RNAseq{chr,1}(val2) = chr_CNVdata_RNAseq{chr,1}(val2) + count1*fragLength2;
						chr_CNVdata_RNAseq{chr,1}(val2)   = chr_CNVdata_RNAseq{chr,1}(val2) + count1*fragLength2/finalFrag_median;
						chr_CNVdata_RNAseq{chr,2}(val2)   = chr_CNVdata_RNAseq{chr,2}(val2) + fragLength2;
						% parent standard_bins data.
						chr_CNVdata_RNAseq{chr,3}(val2)   = chr_CNVdata_RNAseq{chr,3}(val2) + count2*fragLength2;
						chr_CNVdata_RNAseq{chr,4}(val2)   = chr_CNVdata_RNAseq{chr,4}(val2) + fragLength2;
					end;
				end;
			end;
		end;
	...
	save([main_dir 'users/' user '/projects/' project '/corrected_CNV.project.mat'],'chr_CNVdata_RNAseq');
else
...
for chr = 1:num_chrs
    if (chr_in_use(chr) == 1)
        % project standard_bins data : chr_CNVdata_RNAseq{chr,1}(pos)
        % parent standard_bins data  : chr_CNVdata_RNAseq{chr,3}(pos)

        %% Left-end bin.
        pos                = 1;
        valueCurrent       = chr_CNVdata_RNAseq{chr,1}(pos  );
        valueRight         = chr_CNVdata_RNAseq{chr,1}(pos+1);
        chr_CNVdata_RNAseq{chr,1}(pos) = valueCurrent*2/3       + valueRight/3;
        parentValueCurrent = chr_CNVdata_RNAseq{chr,3}(pos  );
        parentValueRight   = chr_CNVdata_RNAseq{chr,3}(pos+1);
        chr_CNVdata_RNAseq{chr,3}(pos) = parentValueCurrent*2/3 + parentValueRight/3;

        %% Middle bins.
        for pos = 2:(length(chr_CNVdata_RNAseq{chr,1})-1)
            valueLeft      = chr_CNVdata_RNAseq{chr,1}(pos-1);
            valueCurrent   = chr_CNVdata_RNAseq{chr,1}(pos  );
            valueRight     = chr_CNVdata_RNAseq{chr,1}(pos+1);
            chr_CNVdata_RNAseq{chr,1}(pos) = valueLeft/4       + valueCurrent/2       + valueRight/4;
            parentValueLeft      = chr_CNVdata_RNAseq{chr,3}(pos-1);
            parentValueCurrent   = chr_CNVdata_RNAseq{chr,3}(pos  );
            parentValueRight     = chr_CNVdata_RNAseq{chr,3}(pos+1);
            chr_CNVdata_RNAseq{chr,3}(pos) = parentValueLeft/4 + parentValueCurrent/2 + parentValueRight/4;
        end;

        %% Right-end bin.
        pos                = length(chr_CNVdata_RNAseq{chr,1});
        valueRight         = chr_CNVdata_RNAseq{chr,1}(pos-1);
        valueCurrent       = chr_CNVdata_RNAseq{chr,1}(pos  );
        chr_CNVdata_RNAseq{chr,1}(pos) = valueLeft/3       + valueCurrent*2/3;
        parentValueRight   = chr_CNVdata_RNAseq{chr,3}(pos-1);
        parentValueCurrent = chr_CNVdata_RNAseq{chr,3}(pos  );
        chr_CNVdata_RNAseq{chr,3}(pos) = parentValueLeft/3 + parentValueCurrent*2/3;
    end;
end;


%% -----------------------------------------------------------------------------------------
% Convert 'chr_CNVdata_RNAseq' to 'CNVplot2' for saving to 'Common_CNV' file and figure generation.
%-------------------------------------------------------------------------------------------
for chr = 1:num_chrs
    CNVplot2{chr}     = zeros(1,length(chr_CNVdata_RNAseq{chr,1}));
    CNV_tracking{chr} = zeros(1,length(chr_CNVdata_RNAseq{chr,1}));
end;
for chr = 1:num_chrs
    if (chr_in_use(chr) == 1)
        for pos = 1:length(chr_CNVdata_RNAseq{chr,1})
            ...
            if ((chr_CNVdata_RNAseq{chr,2}(pos) == 0) || (chr_CNVdata_RNAseq{chr,4}(pos) == 0))
                % No data elements => null value is plotted.
                CNVplot2{chr}(pos)     = 0;
            else
                % project : sum of data elements is divided by the number of data elements.
                CNVplot1a{chr}(pos)    = chr_CNVdata_RNAseq{chr,1}(pos)/chr_CNVdata_RNAseq{chr,2}(pos);

                % parent  : sum of data elements is divided by the number of data elements.
                CNVplot1b{chr}(pos)    = chr_CNVdata_RNAseq{chr,3}(pos)/chr_CNVdata_RNAseq{chr,4}(pos);
