	...
	if (chr_in_use(chr) == 1)
		if (length(chrCopyNum{chr}) > 1)  % more than one segment, so lets examine if adjacent segments have different copyNums.
			...
			if (breakCount_new > 0)
				...
				chrCopyNum_new{chr} = chrCopyNum{chr};
			end;
	...
	if (chr_in_use(chr) == 1)
		...
		if (useHapmap)
			...
			for i = 1:length(dataX)
				...
				if (datumY_2 > 0)
					plot([datumX datumX], [0 maxY],'Color',[chr_SNPdata_colors{chr,1}(i) chr_SNPdata_colors{chr,2}(i) chr_SNPdata_colors{chr,3}(i)]);
				end;
			end;
		else
			dataX      = 1:ceil(chr_size(chr)/new_bases_per_bin);
