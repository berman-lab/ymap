...
homolog_a_color = [1.0   0.0   1.0  ]; % homozygous a:magenta.
homolog_b_color = [0.0   1.0   1.0  ]; % homozygous b:cyan.
het_color       = [0.66667 0.66667 0.66667]; % heterozygous.

% colors for display of phased SNP ratio data for different copy number estimates.
	% haploid colors.
	    ...
	    for chr_bin = 1:length(SNPs_to_fullData_ratio{chr})+1;
	        if (chr_bin-1 < length(SNPs_to_fullData_ratio{chr}))
	            ...
	            if (c_tot_post == 0)
					...
					ratioData         = chr_SNPdata{chr,1}{chr_bin};
					localCopyEstimate = round(CNVplot2{chr}(chr_bin));
					if (localCopyEstimate <= 0)
					elseif (localCopyEstimate == 1)
						binCounts = zeros(1,2);	
						for i = 1:length(ratioData)
							    if (ratioData(i) < 1/2);   binCounts(1) = binCounts(1)+1;
							elseif (ratioData(i) > 1/2);   binCounts(2) = binCounts(2)+1;
							else
								binCounts(1) = binCounts(1)+0.5;
								binCounts(2) = binCounts(2)+0.5;
							end;
						end;
						colorMix = colorA * binCounts(1)/(sum(binCounts)) + ...
						           colorB * binCounts(2)/(sum(binCounts));
					elseif (localCopyEstimate == 2)
						binCounts = zeros(1,3);
						for i = 1:length(ratioData)
							    if (ratioData(i) < 1/4);   binCounts(1) = binCounts(1)+1;
							elseif (ratioData(i) > 3/4);   binCounts(3) = binCounts(3)+1;
							else                           binCounts(2) = binCounts(2)+1;
							end;
						end;
						colorMix = colorAA * binCounts(1)/(sum(binCounts)) + ...
						           colorAB * binCounts(2)/(sum(binCounts)) + ...
						           colorBB * binCounts(3)/(sum(binCounts));
					elseif (localCopyEstimate == 3)
						binCounts = zeros(1,4);
						for i = 1:length(ratioData)
							    if (ratioData(i) < 1/6);  binCounts(1) = binCounts(1)+1;
							elseif (ratioData(i) < 1/2);  binCounts(2) = binCounts(2)+1;
							elseif (ratioData(i) > 5/6);  binCounts(4) = binCounts(4)+1;
							elseif (ratioData(i) > 1/2);  binCounts(3) = binCounts(3)+1;
							else
								binCounts(2) = binCounts(2)+0.5;
								binCounts(3) = binCounts(3)+0.5;
							end;
						end;
						colorMix = colorAAA * binCounts(1)/(sum(binCounts)) + ...
						           colorAAB * binCounts(2)/(sum(binCounts)) + ...
						           colorABB * binCounts(3)/(sum(binCounts)) + ...
						           colorBBB * binCounts(4)/(sum(binCounts));
					elseif (localCopyEstimate == 4)
						binCounts = zeros(1,5);
						for i = 1:length(ratioData)
							    if (ratioData(i) < 1/8);  binCounts(1) = binCounts(1)+1;
							elseif (ratioData(i) < 3/8);  binCounts(2) = binCounts(2)+1;
							elseif (ratioData(i) > 5/8);  binCounts(5) = binCounts(5)+1;
							elseif (ratioData(i) > 7/8);  binCounts(4) = binCounts(4)+1;
							else                          binCounts(3) = binCounts(3)+1;
							end;
						end;
						colorMix = colorAAAA * binCounts(1)/(sum(binCounts)) + ...
						           colorAAAB * binCounts(2)/(sum(binCounts)) + ...
						           colorAABB * binCounts(3)/(sum(binCounts)) + ...
						           colorABBB * binCounts(4)/(sum(binCounts)) + ...
						           colorBBBB * binCounts(5)/(sum(binCounts));
					elseif (localCopyEstimate == 5)
						binCounts = zeros(1,6);
						for i = 1:length(ratioData)
							    if (ratioData(i) < 1/10);  binCounts(1) = binCounts(1)+1;
							elseif (ratioData(i) < 3/10);  binCounts(2) = binCounts(2)+1;
							elseif (ratioData(i) < 1/2);   binCounts(3) = binCounts(3)+1;
							elseif (ratioData(i) > 9/10);  binCounts(6) = binCounts(6)+1;
							elseif (ratioData(i) > 7/10);  binCounts(5) = binCounts(5)+1;
							elseif (ratioData(i) > 1/2);   binCounts(4) = binCounts(4)+1;
							else
								binCounts(3) = binCounts(3)+0.5;
								binCounts(4) = binCounts(4)+0.5;
							end;
						end;
						colorMix = colorAAAAA * binCounts(1)/(sum(binCounts)) + ...
						           colorAAAAB * binCounts(2)/(sum(binCounts)) + ...
						           colorAAABB * binCounts(3)/(sum(binCounts)) + ...
						           colorAABBB * binCounts(4)/(sum(binCounts)) + ...
						           colorABBBB * binCounts(5)/(sum(binCounts)) + ...
						           colorBBBBB * binCounts(6)/(sum(binCounts));
					elseif (localCopyEstimate == 6)
						binCounts = zeros(1,7);
						for i = 1:length(ratioData)
							    if (ratioData(i) < 1/12);  binCounts(1) = binCounts(1)+1;
							elseif (ratioData(i) < 3/12);  binCounts(2) = binCounts(2)+1;
							elseif (ratioData(i) < 5/12);  binCounts(3) = binCounts(3)+1;
							elseif (ratioData(i) > 11/12); binCounts(7) = binCounts(7)+1;
							elseif (ratioData(i) > 9/12);  binCounts(6) = binCounts(6)+1;
							elseif (ratioData(i) > 7/12);  binCounts(5) = binCounts(5)+1;
							else                           binCounts(4) = binCounts(4)+1;
							end;
						end;
						colorMix = colorAAAAAA * binCounts(1)/(sum(binCounts)) + ...
						           colorAAAAAB * binCounts(2)/(sum(binCounts)) + ...
						           colorAAAABB * binCounts(3)/(sum(binCounts)) + ...
						           colorAAABBB * binCounts(4)/(sum(binCounts)) + ...
						           colorAABBBB * binCounts(5)/(sum(binCounts)) + ...
						           colorABBBBB * binCounts(6)/(sum(binCounts)) + ...
						           colorBBBBBB * binCounts(7)/(sum(binCounts));
					elseif (localCopyEstimate == 7)
						binCounts = zeros(1,8);
						for i = 1:length(ratioData)
							    if (ratioData(i) < 1/14);  binCounts(1) = binCounts(1)+1;
							elseif (ratioData(i) < 3/14);  binCounts(2) = binCounts(2)+1;
							elseif (ratioData(i) < 5/14);  binCounts(3) = binCounts(3)+1;
							elseif (ratioData(i) < 7/14);  binCounts(4) = binCounts(4)+1;
							elseif (ratioData(i) > 13/14); binCounts(8) = binCounts(8)+1;
							elseif (ratioData(i) > 11/14); binCounts(7) = binCounts(7)+1;
							elseif (ratioData(i) > 9/14);  binCounts(6) = binCounts(6)+1;
							elseif (ratioData(i) > 7/14);  binCounts(5) = binCounts(5)+1;
							else
								binCounts(4) = binCounts(4)+0.5;
								binCounts(5) = binCounts(5)+0.5;
							end;
						end;
						colorMix = colorAAAAAAA * binCounts(1)/(sum(binCounts)) + ...
						           colorAAAAAAB * binCounts(2)/(sum(binCounts)) + ...
						           colorAAAAABB * binCounts(3)/(sum(binCounts)) + ...
						           colorAAAABBB * binCounts(4)/(sum(binCounts)) + ...
						           colorAAABBBB * binCounts(5)/(sum(binCounts)) + ...
						           colorAABBBBB * binCounts(6)/(sum(binCounts)) + ...
						           colorABBBBBB * binCounts(7)/(sum(binCounts)) + ...
						           colorBBBBBBB * binCounts(8)/(sum(binCounts));
					elseif (localCopyEstimate == 8)
						binCounts = zeros(1,9);
						for i = 1:length(ratioData)
							    if (ratioData(i) < 1/16);  binCounts(1) = binCounts(1)+1;
							elseif (ratioData(i) < 3/16);  binCounts(2) = binCounts(2)+1;
							elseif (ratioData(i) < 5/16);  binCounts(3) = binCounts(3)+1;
							elseif (ratioData(i) < 7/16);  binCounts(4) = binCounts(4)+1;
							elseif (ratioData(i) > 15/16); binCounts(9) = binCounts(9)+1;
							elseif (ratioData(i) > 13/16); binCounts(8) = binCounts(8)+1;
							elseif (ratioData(i) > 11/16); binCounts(7) = binCounts(7)+1;
							elseif (ratioData(i) > 9/16);  binCounts(6) = binCounts(6)+1;
							else                           binCounts(5) = binCounts(5)+1;
							end;
						end;
						colorMix = colorAAAAAAAA * binCounts(1)/(sum(binCounts)) + ...
						           colorAAAAAAAB * binCounts(2)/(sum(binCounts)) + ...
						           colorAAAAAABB * binCounts(3)/(sum(binCounts)) + ...
						           colorAAAAABBB * binCounts(4)/(sum(binCounts)) + ...
						           colorAAAABBBB * binCounts(5)/(sum(binCounts)) + ...
						           colorAAABBBBB * binCounts(6)/(sum(binCounts)) + ...
						           colorAABBBBBB * binCounts(7)/(sum(binCounts)) + ...
						           colorABBBBBBB * binCounts(8)/(sum(binCounts)) + ...
						           colorBBBBBBBB * binCounts(9)/(sum(binCounts));
					elseif (localCopyEstimate >= 9)
						binCounts = zeros(1,10);
						for i = 1:length(ratioData)
							    if (ratioData(i) < 1/18);  binCounts(1)  = binCounts(1) +1;
							elseif (ratioData(i) < 3/18);  binCounts(2)  = binCounts(2) +1;
							elseif (ratioData(i) < 5/18);  binCounts(3)  = binCounts(3) +1;
							elseif (ratioData(i) < 7/18);  binCounts(4)  = binCounts(4) +1;
							elseif (ratioData(i) < 1/2);   binCounts(5)  = binCounts(5) +1;
							elseif (ratioData(i) > 17/18); binCounts(10) = binCounts(10)+1;
							elseif (ratioData(i) > 15/18); binCounts(9)  = binCounts(9) +1;
							elseif (ratioData(i) > 13/18); binCounts(8)  = binCounts(8) +1;
							elseif (ratioData(i) > 11/18); binCounts(7)  = binCounts(7) +1;
							elseif (ratioData(i) > 1/2);   binCounts(6)  = binCounts(6) +1;
							else
								binCounts(5) = binCounts(5)+0.5;
								binCounts(6) = binCounts(6)+0.5;
							end;
						end;
						colorMix = colorAAAAAAAAA * binCounts(1 )/(sum(binCounts)) + ...
						           colorAAAAAAAAB * binCounts(2 )/(sum(binCounts)) + ...
						           colorAAAAAAABB * binCounts(3 )/(sum(binCounts)) + ...
						           colorAAAAAABBB * binCounts(4 )/(sum(binCounts)) + ...
						           colorAAAAABBBB * binCounts(5 )/(sum(binCounts)) + ...
						           colorAAAABBBBB * binCounts(6 )/(sum(binCounts)) + ...
						           colorAAABBBBBB * binCounts(7 )/(sum(binCounts)) + ...
						           colorAABBBBBBB * binCounts(8 )/(sum(binCounts)) + ...
						           colorABBBBBBBB * binCounts(9 )/(sum(binCounts)) + ...
						           colorBBBBBBBBB * binCounts(10)/(sum(binCounts));
					end;
