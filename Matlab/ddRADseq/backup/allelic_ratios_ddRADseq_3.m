...
if (useHapmap)
	for chr = 1:num_chrs
		if (dataReductionMethod == 1)
			% experiment : data reduction by averaging colors within each bin.
	    ...
	    if (Linear_display == true)
			...
			if (useHapmap)
				...
				for i = 1:length(dataX)
					...
					datumY_1 = dataY_1(i)*maxY;
					datumY_2 = maxY - dataY_2(i)*maxY;
					plot([datumX datumX], [maxY     datumY_1],'Color',[chr_SNPdata_colorsC{chr,1}(i) chr_SNPdata_colorsC{chr,2}(i) chr_SNPdata_colorsC{chr,3}(i)]);
					plot([datumX datumX], [0        datumY_2],'Color',[chr_SNPdata_colorsC{chr,1}(i) chr_SNPdata_colorsC{chr,2}(i) chr_SNPdata_colorsC{chr,3}(i)]);
					if (datumY_2 > 0)
						plot([datumX datumX], [datumY_1 datumY_2],'Color',[chr_SNPdata_colorsC{chr,1}(i) chr_SNPdata_colorsC{chr,2}(i) chr_SNPdata_colorsC{chr,3}(i)]);
					end;
				end;
			else
				dataX      = 1:ceil(chr_size(chr)/new_bases_per_bin);
