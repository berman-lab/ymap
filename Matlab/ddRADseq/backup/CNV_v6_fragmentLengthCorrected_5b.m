...
if (ParentValid == 1)
	...
	for fragID = 1:numFragments
		...
		fragment_data(fragID).usable = 1;
		if (Y_fitCurve1_parent(fragID)         <= 1   );   fragment_data(fragID).usable = 0;   end;
		if (Y_reads_parent(fragID)             <= 1.5 );   fragment_data(fragID).usable = 0;   end;
		if (Y_reads_parent_corrected_1(fragID) <= 0   );   fragment_data(fragID).usable = 0;   end;
...
for fragID = 1:numFragments
	...
	if (ParentValid == 1)
		...
		Y_reads_parent(fragID)  = fragment_data(fragID).ave_reads_corrected_1;
	end;
	usable_project(fragID)      = fragment_data(fragID).usable;
end;
X_GCbias_trimmed        = X_GCbias;
Y_reads_project_trimmed = Y_reads_project;
Y_reads_parent_trimmed  = Y_reads_parent;
%------------------------------------------------------------*
% Trim dataset before LOWESS fitting 2.                      |
%------------------------------------------------------------*
	% Remove fragments previously noted as not usable.
	X_GCbias_trimmed(        usable_project == 0) = [];
	Y_reads_project_trimmed( usable_project == 0) = [];
	Y_reads_parent_trimmed(  usable_project == 0) = [];
%------------------------------------------------------------*
...
[newX2_project, newY2_project] = optimize_mylowess(X_GCbias_trimmed,Y_reads_project_trimmed,10);
fprintf('\tLOWESS fitting to project data complete.\n');
% Calculate GC_bias_corrected length_bia_corrected ave_read_count data for plotting and later analysis.
Y_target                    = 1;
Y_fitCurve2_project         = interp1(newX2_project,newY2_project,X_GCbias,'spline');
...
if (ParentValid == 1)
	...
	[newX2_parent, newY2_parent] = optimize_mylowess(X_GCbias_trimmed,Y_reads_parent_trimmed,10);
	fprintf('\tLOWESS fitting to project data complete.\n');
	% Calculate GC_bias_corrected length_bia_corrected ave_read_count data for plotting and later analysis.
	Y_target                   = 1;
...
hold off;
axis normal;
h = title('GC ratio bias corrected. [EXPERIMENT]');   set(h, 'FontSize', 5);
...
if (ParentValid == 1)
	...
	hold off;
	axis normal;
	h = title('GC ratio bias corrected. [REF]');   set(h, 'FontSize', 5);
	h = ylabel('Corrected Reads 2');               set(h, 'FontSize', 5);
	h = xlabel('GC ratio');                        set(h, 'FontSize', 5);
	set(gca,'FontSize',5);
	xlim([0 1]);
...
if (ParentValid == 1)
	...
	hold off;
	axis normal;
	h = title('Corrected_2 reads count vs. ddRADseq fragment length. [REF]');   set(h, 'FontSize', 5);
	h = ylabel('Corrected reads');   set(h, 'FontSize', 5);
	h = xlabel('Fragment Length');   set(h, 'FontSize', 5);
	set(gca,'FontSize',5);
	ylim([0 4]);
...
for fragID = 1:numFragments
	% Add corrected ave_data to fragment data structure.
	if (ParentValid == 1)
		fragment_data(fragID).ave_reads_corrected_3 = Y_reads_project_corrected_2(fragID)/Y_reads_parent_corrected_2(fragID);
	else
		fragment_data(fragID).ave_reads_corrected_3 = 1;
	end;
...
%-------------------------------------------------------------------------------------------------
% Distribute project data into standard bins, then save as 'corrected_CNV.mat'.
%-------------------------------------------------------------------------------------------------
if (exist([main_dir 'users/' user '/projects/' project '/corrected_CNV.mat'],'file') == 0)
	...
	for fragID = 1:numFragments
		...
		% Abbey
		%	count      = fragment_data(fragID).ave_reads;                   % raw data.
		%	count      = fragment_data(fragID).ave_reads_corrected_1;       % normalized vs. fragment_length.
		%	count      = fragment_data(fragID).ave_reads_corrected_2;       % normalized vs. fragment_length and fragment_GCbias.
			if (ParentValid == 1)
				count      = fragment_data(fragID).ave_reads_corrected_3;   % normalized vs. fragment_length, fragment_GCbias, and fragment_reference.
			else
				count      = fragment_data(fragID).ave_reads_corrected_2;
			end;
...
% #####################################################
% # Darren : I don't know what this section is for... #
% #####################################################
%	%% -----------------------------------------------------------------------------------------
...
%		count   = fragment_data(fragID).ave_reads_corrected_3;
%	
%		if (Low_quality_ploidy_estimate == true)
%			string4 = num2str(count*ploidy*ploidyAdjust);
%		else
%			string4 = num2str(count*ploidy);
%		end;
