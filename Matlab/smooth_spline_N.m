function [z,s,exitflag,Wtot] = smooth_spline_N(varargin)
...
if ~isequal(size(W),sizy)
	error('MATLAB:smoothn:SizeMismatch', 'Arrays for data and weights must have same size.')
elseif ~isempty(s) && (~isscalar(s) || s<0)
	error('MATLAB:smoothn:IncorrectSmoothingParameter', 'The smoothing parameter must be a scalar >=0')
end
%---
...
if isempty(I)
	...
	try
		...
		error('MATLAB:smoothn:IncorrectMaxIter', 'MaxIter must be an integer >=1')
	end
...
if isempty(I)
	...
	try
		...
		error('MATLAB:smoothn:IncorrectTolZ', 'TolZ must be in ]0,1[')
	end
...
if isempty(I)
	...
	try
		...
		error('MATLAB:smoothn:IncorrectInitialGuess', 'Z0 must be a valid initial guess for Z')
	end
	if ~isnumeric(z0) || ~isequal(size(z0),sizy)
		error('MATLAB:smoothn:IncorrectTolZ', 'Z0 must be a valid initial guess for Z')
	end
end
%---
...
if isempty(I)
	...
	try
		...
		error('MATLAB:smoothn:IncorrectWeights', 'A valid weighting function must be chosen')
	end
	if ~ischar(weightstr)
		error('MATLAB:smoothn:IncorrectWeights', 'A valid weighting function must be chosen')
	end
...
W = W.*IsFinite;
if any(W<0)
	error('MATLAB:smoothn:NegativeWeights', 'Weights must all be >=0')
else 
	W = W/max(W(:));
end
%---
...
for i = 1:d
	siz0 = ones(1,d);
	siz0(i) = sizy(i);
	Lambda = bsxfun(@plus,Lambda, cos(pi*(reshape(1:sizy(i),siz0)-1)/sizy(i)));
end
Lambda = -2*(d-Lambda);
...
if isauto
	if abs(log10(s)-log10(sMinBnd))<errp
		warning('MATLAB:smoothn:SLowerBound', ['s = ' num2str(s,'%.3e') ': the lower bound for s ', 'has been reached. Put s as an input variable if required.'])
	elseif abs(log10(s)-log10(sMaxBnd))<errp
		warning('MATLAB:smoothn:SUpperBound', ['s = ' num2str(s,'%.3e') ': the upper bound for s ', 'has been reached. Put s as an input variable if required.'])
	end
end
if nargout<3 && ~exitflag
	warning('MATLAB:smoothn:MaxIter', ['Maximum number of iterations (' int2str(MaxIter) ') has ', 'been exceeded. Increase MaxIter option or decrease TolZ value.'])
end
...
function test4DCTNandIDCTN
	if ~exist('dctn','file')
		error('MATLAB:smoothn:MissingFunction', ['DCTN and IDCTN are required. Download DCTN <a href="matlab:web(''', 'http://www.biomecardio.com/matlab/dctn.html'')">here</a>.'])
	elseif ~exist('idctn','file')
		error('MATLAB:smoothn:MissingFunction', ['DCTN and IDCTN are required. Download IDCTN <a href="matlab:web(''', 'http://www.biomecardio.com/matlab/idctn.html'')">here</a>.'])
	end
end
