function [] = CNV_v6_fragmentLengthCorrected_7(main_dir,user,genomeUser,project,parent,genome,ploidyEstimate,ploidyBase, ...
...
test1 = length(project_fragments_CNV)
...
%-------------------------------------------------------------------------------------------------
fprintf(['RNAseq_bins_GC_ratios_file :\n\t' main_dir 'users/' genomeUser '/genomes/' genome '/' FastaName '.GC_ratios.expression.txt\n']);
RNAseq_GC_ratios_fid = fopen([main_dir 'users/' genomeUser '/genomes/' genome '/' FastaName '.GC_ratios.expression.txt'], 'r');
fprintf(['\t' num2str(RNAseq_GC_ratios_fid) '\n']);
fragID = 0;
while not (feof(RNAseq_GC_ratios_fid))
	dataLine = fgetl(RNAseq_GC_ratios_fid);
	if (length(dataLine) > 0)
		if (dataLine(1) ~= '#')
			% The number of valid lines found so far...  the number of usable restriction fragments with data so far.
			fragID              = fragID + 1;
...
fclose(RNAseq_GC_ratios_fid);


%%================================================================================================
% Load pre-processed RNAseq fragment repetitiveness data for genome.
%-------------------------------------------------------------------------------------------------
fprintf(['RNAseq_bins_repetitiveness_file :\n\t' main_dir 'users/' genomeUser '/genomes/' genome '/' FastaName '.repetitiveness.expression.txt\n']);
repetitiveness_fid = fopen([main_dir 'users/' genomeUser '/genomes/' genome '/' FastaName '.repetitiveness.expression.txt'], 'r');
...
for fragID = 1:numFragments
	...
	fragment_data(fragID).repetitiveness = repetitivenessData(fragID);
...
for fragID = 1:numFragments
%	test1 = fragment_data(fragID).length;
%	test2 = fragment_data(fragID).ave_reads;
%	test3 = fragment_data(fragID).ave_reads_parent;
%	fprintf([num2str(fragID) '\t' num2str(test1) '\t' num2str(test2) '\t' num2str(test3) '\n']);
...
%	Y_reads_project_corrected_1 = Y_reads_project./Y_fitCurve1_project*Y_target;
	...
	%-------------------------------------------------------------------------------------------------
%	for fragID = 1:numFragments
%		X_datum = X_length(fragID);
%		Y_datum = Y_reads_project(fragID);
...
%	end;
	...
	h = ylabel('log_2(Average Reads)');     set(h, 'FontSize', 10);
...
if (performGCbiasCorrection)
	...
	hold off;
	axis normal;
	h = title('Corrected Read count vs. RNAseq fragment length. [EXP]');   set(h, 'FontSize', 10);
	h = ylabel('Corrected reads');   set(h, 'FontSize', 10);
	h = xlabel('Fragment Length');   set(h, 'FontSize', 10);
	set(gca,'FontSize',10);
%	ylim([0 4]);
	...
	if (useParent)
		...
		hold off;
		axis normal;
		h = title('Corrected_2 reads count vs. RNAseq fragment length. [REF]');   set(h, 'FontSize', 10);
		h = ylabel('Corrected reads');   set(h, 'FontSize', 10);
		h = xlabel('Fragment Length');   set(h, 'FontSize', 10);
		set(gca,'FontSize',10);
%		ylim([0 4]);
