...
fprintf(['\nSnpCghArray:Load_genome_information_1.m running.\n']);
...
centromeres = [];
centromere_fid = fopen(['genome_details/centromere_locations.txt'], 'r');
...
%       respect to each chromosome.
chrSize = [];
chrSize_fid = fopen(['genome_details/chromosome_sizes.txt'],'r')
...
%       respect to each chromosome.
annotations = [];
annotations_fid = fopen(['genome_details/annotations.txt'], 'r');
...
figure_details = [];
figInfo_fid = fopen(['genome_details/figure_definitions.txt'], 'r');
...
while not (feof(figInfo_fid))
    ...
    figInfo_useChr = sscanf(line, '%s',2);
    for i = 1:size(sscanf(line,'%s',1),2);
        figInfo_useChr(1) = [];
    end;
    figInfo_label = sscanf(line, '%s',3);
    for i = 1:size(sscanf(line,'%s',2),2);
        figInfo_label(1) = [];
    end;
	...
	lines_analyzed = lines_analyzed+1;
	figInfo_name   = sscanf(line, '%s',4);
	for i = 1:size(sscanf(line,'%s',3),2);
	    figInfo_name(1) = [];
	end;
	figInfo_posX   = sscanf(line, '%s',5);
	for i = 1:size(sscanf(line,'%s',4),2);
	    figInfo_posX(1) = [];
	end;
	figInfo_posY   = sscanf(line, '%s',6);
	for i = 1:size(sscanf(line,'%s',5),2);
	    figInfo_posY(1) = [];
	end;
	figInfo_width   = sscanf(line, '%s',7);
	for i = 1:size(sscanf(line,'%s',6),2);
	    figInfo_width(1) = [];
	end;
	figInfo_height   = sscanf(line, '%s',8);
	for i = 1:size(sscanf(line,'%s',7),2);
	    figInfo_height(1) = [];
	end;
	...
	figInfo_ploidy_default = sscanf(line, '%s',4);
	for i = 1:size(sscanf(line,'%s',3),2);
