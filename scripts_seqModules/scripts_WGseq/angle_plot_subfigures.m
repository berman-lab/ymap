		...
		for region = mostLikelyGaussians
			...
			if (length(mostLikelyGaussians) <= 1)
				...
				if (region_ == 1) % first region in list.
					coord1 = min(round(actual_cutoffs(region_))+1, 200);
				...
				elseif (region_ == length(mostLikelyGaussians)) % last region in list.
					coord2 = min(round(actual_cutoffs(region_-1))+1, 200);
					...
					coord3 = min(round(actual_cutoffs(region_-1))+1, 200);
					coord4 = min(round(actual_cutoffs(region_  ))+1, 200);
