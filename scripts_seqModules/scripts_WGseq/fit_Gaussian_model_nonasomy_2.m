function [p1_a,p1_b,p1_c, p2_a,p2_b,p2_c, p3_a,p3_b,p3_c, p4_a,p4_b,p4_c, p5_a,p5_b,p5_c, p6_a,p6_b,p6_c, p7_a,p7_b,p7_c, p8_a,p8_b,p8_c, p9_a,p9_b,p9_c, p10_a,p10_b,p10_c, skew_factor] = fit_Gaussian_model_nonasomy_2(workingDir, saveName, data,locations,init_width,func_type)
	% attempt to fit a 10-gaussian model to data.
	...
	fprintf(['||3 datamax = ' num2str(datamax) '\n']);
	...
	p9_ai  = datamax;   p9_bi  = locations(8);   p9_ci  = init_width;
	p10_ai = datamax;   p10_bi = locations(8);   p10_ci = init_width;

	initial = [p1_ai,p1_ci,p2_ai,p2_ci,p3_ai,p3_ci,p4_ai,p4_ci,p5_ai,p5_ci,p6_ai,p6_ci,p7_ai,p7_ci,p8_ai,p8_ci,p9_ai,p9_ci,p10_ai,p10_ci,  skew_factor,skew_factor,skew_factor,skew_factor,skew_factor,skew_factor,skew_factor,skew_factor,skew_factor,skew_factor];
	...
	p10_a         = abs(Estimates(19));
        p10_b         = locations(10);
        p10_c         = abs(Estimates(20));
	skew_factor1  = abs(Estimates(21));
	skew_factor2  = abs(Estimates(22));
	skew_factor3  = abs(Estimates(23));
	skew_factor4  = abs(Estimates(24));
	skew_factor5  = abs(Estimates(25));
	skew_factor6  = abs(Estimates(26));
	skew_factor7  = abs(Estimates(27));
	skew_factor8  = abs(Estimates(28));
	skew_factor9  = abs(Estimates(29));
	skew_factor10 = abs(Estimates(30));
	...
	if (skew_factor10 < 0); skew_factor10 = 0; end; if (skew_factor10 > 2); skew_factor10 = 2; end;
	...
	p9_c  = p9_c*p9_c/c9_;
	c10_  = p10_c/2 + p10_c*skew_factor10/(100.5-abs(100.5-p10_b))/2;
	p10_c = p10_c*p10_c/c10_;
end
...
function sse = fiterror(params,time,data,func_type,locations,show)
	...
	skew_factor4  = abs(params(24));
	skew_factor5  = abs(params(25));
	skew_factor6  = abs(params(26));
	skew_factor7  = abs(params(27));
	skew_factor8  = abs(params(28));
	skew_factor9  = abs(params(29));
	skew_factor10 = abs(params(30));
	...
	time10_1 = 1:floor(p10_b);
        time10_2 = ceil(p10_b):200;
        if (time10_1(end) == time10_2(1));    time10_2(1) = [];    end;
	c1_   = p1_c/2 + p1_c*skew_factor1/(100.5-abs(100.5-p1_b))/2;
	p1_c  = p1_c*p1_c/c1_;
	c2_   = p2_c/2 + p2_c*skew_factor2/(100.5-abs(100.5-p2_b))/2;
	p2_c  = p2_c*p2_c/c2_;        
	...
	p5_fit_L  = p5_a*exp( -0.5*((time5_1 -p5_b )./p5_c ).^2);
        p5_fit_R  = p5_a*exp( -0.5*((time5_2 -p5_b )./p5_c /(skew_factor5 /(100.5-abs(100.5-p5_b ))) ).^2);
	...
	p10_fit_L = p10_a*exp(-0.5*((time10_1-p10_b)./p10_c/(skew_factor10/(100.5-abs(100.5-p10_b))) ).^2);
	p10_fit_R = p10_a*exp(-0.5*((time10_2-p10_b)./p10_c).^2);
	...
	p10_fit = [p10_fit_L p10_fit_R];
	fitted = p1_fit+p2_fit+p3_fit+p4_fit+p5_fit+p6_fit+p7_fit+p8_fit+p9_fit+p10_fit;
...
plot(p10_fit,'-','color',[0 0.75 0.75],'lineWidth',2);
