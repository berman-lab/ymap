...
%	Xinput     : Vector (n x 1).
%	Yinput     : Vector (n x 1).
%	f          : span width percentage.
%	wantplot   : if ~0, then make plot.
%	imagefile  : output file image in PNG format.
%	xdata      : alternate x-coordinates to find lowess fit values for.
%
%	dataout    : results of fitting, using secondary x-coordinates if available.
% 		*  dataout(:,1)    = x
%		*  dataout(:,2)    = y
...
%	xy         : secondary output, using primary x-coordinates always.
%		*  xy(:,1)         = x
%		*  xy(:,2)         = y
%====================================================================================================
%
...
function [yy] = rwlreg(x,y,n,d,r,xx)
	...
	ddmax = abs(x(n) - x(1));
	if dd == 0.0
		error('Regression:smooth_lowess','LOWESS window size = 0. Increase f.');
	else
		while dd <= ddmax
			c = 0;
			total = 0.0;
