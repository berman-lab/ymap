...
fprintf('\n');
fprintf('#################################\n');
fprintf('## allelic_ratios_RNAseq_3.m   ##\n');
fprintf('#################################\n');
...
for chr = 1:length(chr_sizes)
	...
	for j = 1:3
		chr_SNPdata_colors{chr,j} = zeros(chr_length,1)+2/3;
	end;
	chr_SNPdata_count{chr,1} = zeros(chr_length,1);
end;
...
%-------------------------------------------------------------------------------------------------
if (useHapmap)
	if (exist([projectDir 'SNP_' SNP_verString '.all2.mat'],'file') == 0)
		fprintf('\nAllelic fraction MAT file 2 not found, generating.\n');
		process_1dataset_hapmap_allelicRatios(projectDir, hapmapDir, chr_size, chr_name, chr_in_use, SNP_verString);
	else
		fprintf('\nAllelic fraction MAT file 2 found, loading.\n');
	end;
	load([projectDir 'SNP_' SNP_verString '.all2.mat']);
else
...
if (useHapmap)
	for chr = 1:num_chrs
		if (true)
			% data reduction by averaging colors within each bin.
			for i = 1:length(C_chr_count{chr})
				...
				chr_SNPdata_colors{chr,1}(pos) = colorList(1);
				chr_SNPdata_colors{chr,2}(pos) = colorList(2);
				chr_SNPdata_colors{chr,3}(pos) = colorList(3);
				chr_SNPdata_count{ chr,1}(pos) = chr_SNPdata_count{chr,1}(pos)+1;
			end;
			for i = 1:length(C_chr_count{chr})
				if (chr_SNPdata_count{chr,1}(pos) > 0)
					chr_SNPdata_colors{chr,1}(pos) = chr_SNPdata_colors{chr,1}(pos)/chr_SNPdata_count{chr,1}(pos);
					chr_SNPdata_colors{chr,2}(pos) = chr_SNPdata_colors{chr,2}(pos)/chr_SNPdata_count{chr,1}(pos);
					chr_SNPdata_colors{chr,3}(pos) = chr_SNPdata_colors{chr,3}(pos)/chr_SNPdata_count{chr,1}(pos);
				end;
			end;
		else
			% data reduction by only looking at closest to 1:1 data per bin.
			for i = 1:length(C_chr_count{chr})
				pos = ceil(C_chr_SNP_data_positions{chr}(i)/new_bases_per_bin);
	...
	for chr = 1:num_chrs
		for i = 1:length(C_chr_count{chr})
			...
			if (C_chr_SNP_data_ratios{chr}(i) < chr_SNPdata{chr,2}(pos))
				...
				chr_SNPdata_colors{chr,1}(i) = colorList(1);
				chr_SNPdata_colors{chr,2}(i) = colorList(2);
				chr_SNPdata_colors{chr,3}(i) = colorList(3);
			end;
		end;
...
save([projectDir 'SNP_' SNP_verString '.reduced.mat'],'chr_SNPdata','new_bases_per_bin','chr_SNPdata_colors');
