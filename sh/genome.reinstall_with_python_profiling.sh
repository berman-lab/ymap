...
user="darren1";
genome="C_albicans_SC5314_vA21-s02-m09-r07_python_profiling";
...
then
	...
	then
		...
		echo "" > $outputFile;
		python -m cProfile $main_dir"py/genome/genome_process_for_RADseq.repetitiveness_1.py" $inputFile $reflocation $ddRADseq_FASTA1 $logName >> $outputFile;
	fi
	...
	then
		...
		inputFile=$reflocation$FASTAname".repetitiveness.txt";
		echo "" > $outputFile;
		python -m cProfile $main_dir"py/genome/genome_process_for_RADseq.repetitiveness_1.py" $inputFile $reflocation $ddRADseq_FASTA2 $logName >> $outputFile;
	fi
fi
