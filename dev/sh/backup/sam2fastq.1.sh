...
do
	read seq_line <&5;
	new_seq_line=$seq_line;
	new_seq_line=$(sh $main_dir"sh/SEQUENCE_reverse_complement.sh" $seq_line);
	read Q_id_line <&5;
	read qual_line <&5;
	new_qual_line=$(echo $qual_line|rev);
	echo $S_id_line"/2" >> $output2;
	echo $new_seq_line  >> $output2;
	echo $Q_id_line     >> $output2;
	echo $new_qual_line >> $output2;
done
exec 5<&-;
) &
