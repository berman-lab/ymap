...
then
	echo;
	echo "# Command syntax is : 'sh FASTQ_filterShortReads.sh [dataset] [cutoff] [outfile]'";
	echo "# ";
	echo "#        [dataset] : FASTQ read file input.";
	echo "#        [cutoff]  : Reads shorter than this are discarded.";
	...
	do
		...
		if [ $lengthOfRead -ge $filterCutoff ]
		then
			echo $line1_1 >> $3;
			echo $line1_2 >> $3;
			echo $line1_3 >> $3;
