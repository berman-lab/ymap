...
<?php
	...
	if (file_exists($directory."users/".$user."/genomes/".$genome."/complete.txt")) {
		?>
		<html>
		<body onload = "parent.update_genome_label_color('<?php echo $key; ?>','#00AA00'); parent.update_genome_remove_iframe('<?php echo $key; ?>');">
		</body>
		</html>
		<?php
	} else if (file_exists($directory."users/".$user."/genomes/".$genome."/working.txt")) {
		// Load last line from "condensed_log.txt" file.
		$condensedLog      = explode("\n", trim(file_get_contents($GLOBALS['directory']."users/".$user."/genomes/".$genome."/condensed_log.txt")));
		...
		<BODY onload = "parent.resize_iframe('<?php echo $key; ?>', 20*2+12);" class="tab">
			<font color="red"><b>[Processing uploaded data.]</b></font><?php echo " &nbsp; &nbsp; ".$clock; ?><br>
			<?php
