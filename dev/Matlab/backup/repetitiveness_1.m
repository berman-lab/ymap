...
if (exist([workingDir genome '_seq.mat'],'file') == 0) && (exist([workingDir genome '_seqRevCom.mat'],'file') == 0)
    ...
    save([workingDir genome '_seq.mat']      ,'sequences', 'names');
...
if (exist([workingDir genome '_nmerCounts_' num2str(nmer_length) '.mat'],'file') == 0)
...
%	disp(names{chr});
    ...
    save([workingDir genome '_nmerCounts_' num2str(nmer_length) '.mat'],'nmer_counts');
    ...
    load([workingDir genome '_nmerCounts_' num2str(nmer_length) '.mat']);
end;

%% ====================================================================
% Determine distribution of repeats across genome.
% ---------------------------------------------------------------------
if (exist([workingDir genome '_nmerDists_' num2str(nmer_length) '.mat'],'file') == 0)
...
%		disp(names{chr});
    ...
    save([workingDir genome '_nmerDists_' num2str(nmer_length) '.mat'],'repeats');
    ...
    load([workingDir genome '_nmerDists_' num2str(nmer_length) '.mat']);
    for chr = 1:num_chr
...
outFile = [workingDir genome '_repetitiveness.txt'];
fid = fopen(outFile,'w');
...
%    fprintf(']');
%
%    outFile = [workingDir genome '_repetitiveness.sigma_' num2str(sigma) '.txt'];
%    fid = fopen(outFile,'w');
%    fprintf(fid,'## Repetitiveness score per bp location.\n');
