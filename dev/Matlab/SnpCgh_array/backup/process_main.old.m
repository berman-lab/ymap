function [result_image_location, archive_data_location] = ...
    process_main(microarray_design, data_file, header_rows, probeName_col, data_col_ch1, ...
                 data_col_ch2, data_col_ratio, data_col_log2ratio, phasing_dataset, ploidy_estimate, ploidy_base, image_format, ...
...
fprintf(['    11 : ploidy baseline      : ' ploidy_base        '\n']);
...
ploidy_base         = round(str2num(ploidy_base));
if (ploidy_base > 4);   ploidy_base = 4;   end;
if (ploidy_base < 1);   ploidy_base = 1;   end;
fprintf(['\nEuploid base = "' num2str(ploidy_base) '"\n']);
...
for chromosome = 1:num_chrs
	...
	usedPlotCGH = CNVplot2{chromosome}*ploidy_estimate;
	...
	if (Linear_display == true)
		...
		if (show_MRS)
			...
			for i = 1:length(MRS_location)
				if (MRS_chr(i) == chromosome)
					MRSloc = MRS_location(i)*chr_length_scale_multiplier-0.5*(5000/bases_per_bin);
					plot(MRSloc,-maxY/10*1.5,'k:o','MarkerEdgeColor','k','MarkerFaceColor','k','MarkerSize',3)
				end;
			end;
			hold off;
		end;
