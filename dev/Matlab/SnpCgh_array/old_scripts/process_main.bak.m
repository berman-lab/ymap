...
bases_per_bin = round(max(chr_size)/700);   %% = 4555
...
%% ====================================================================
% Use ChARM algorithm to determine breakpoints in data.
%----------------------------------------------------------------------
...
%standard_bins_repetitiveness_fid = fopen('standard_bins.repetitiveness.txt', 'r');
%lines_analyzed = 0;
%for chr = 1:num_chrs   % eight chromosomes.
...
%		fragment_start = sscanf(dataLine, '%s',2);  for i = 1:size(sscanf(dataLine,'%s',1),2);      fragment_start(1) = []; end;    fragment_start = str2num(fragment_start);
%		fragment_end   = sscanf(dataLine, '%s',3);  for i = 1:size(sscanf(dataLine,'%s',2),2);      fragment_end(1) = [];   end;    fragment_end   = str2num(fragment_end);
%		repet_max      = sscanf(dataLine, '%s',4);  for i = 1:size(sscanf(dataLine,'%s',3),2);      repet_max(1) = [];      end;    repet_max      = str2num(repet_max);
%		repet_ave      = sscanf(dataLine, '%s',4);	for i = 1:size(sscanf(dataLine,'%s',3),2);		repet_ave(1) = [];      end;	repet_ave      = str2num(repet_ave);
%		position       = ceil(fragment_start/4555);
%		chr_repetitivenessData{chr}(position) = repet_ave;
%	end;
%end;
