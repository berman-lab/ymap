...
matlab_save_dir     = [workingDir 'matlab_dir/'];
...
load([matlab_save_dir '/' strrep(experiment_name,' ','_') '.' microarray_design '.SNP_data.mat']);
load([matlab_save_dir '/' strrep(experiment_name,' ','_') '.' microarray_design '.CGH_data.mat']);
...
%----------------------------------------------------------------------
if (exist([raw_data_dir '/segmental_aneuploidy.txt'],'file') ~= 0)
    fprintf(['\nLoading segmental aneuploidy file from: ' strrep(raw_data_dir,'\','\\')]);
    fid = fopen([raw_data_dir '/segmental_aneuploidy.txt'],'r');
    i = 0;
    segmental_aneuploidy = [];
    while not (feof(fid))
	lineText = fgetl(fid);
	% header lines start with '#', useful lines do not.
	if (lineText(1) ~= '#')
	    % First collumn indicates the chromosome of interest.
	    segAneu_chr   = sscanf(lineText, '%s',1);

	    % Second collumn indicates where a break is found in the chromosome.
	    segAneu_break        = sscanf(lineText, '%s',2);
	    for k = 1:length(sscanf(lineText,'%s',1));
		segAneu_break(1) = [];
	    end;

	    % Third collumn indicates the datafile of interest.
	    segAneu_dataset      = sscanf(lineText, '%s',3);
	    for k = 1:length(sscanf(lineText,'%s',2));
		segAneu_dataset(1) = [];
	    end;

	    if (strcmp(segAneu_dataset,raw_data_file) == 1)
		i = i+1;
...
% Save datasetDetails.
%----------------------------------------------------------------------
save([matlab_save_dir '/' strrep(experiment_name,' ','_') '.' microarray_design '.datasetDetails.mat'], 'datasetDetails');
...
genome_CNV = 'Ca_a';
save([workingDir 'matlab_dir/' strrep(experiment_name,' ','_') '.common_CNV.mat'], 'CNVplot2','genome_CNV');
...
if (Output_CGD_annotations == true)
    CGDid = fopen(['~/' strrep(experiment_name,' ','_') '.txt'], 'w');
    fprintf(CGDid,['track name=' strrep(experiment_name,' ','_') ' description="SNP/CGH microarray annotation of SNPs" useScore=0 itemRGB=On\n']);
end;
...
saveas(fig, ['~/' strrep(experiment_name,' ','_') '.1.eps'], 'epsc');
%saveas(fig, ['~/' strrep(experiment_name,' ','_') '.1.png'], 'png');
delete(fig);

% save then delete linear genome figure, after resizing image output.
set(Linear_fig,'PaperPosition',[0 0 8 0.62222222]);
saveas(Linear_fig, ['~/' strrep(experiment_name,' ','_') '.2.eps'], 'epsc');
%saveas(Linear_fig, ['~/' strrep(experiment_name,' ','_') '.2.png'], 'png');
delete(Linear_fig);
...
result_image_location1 = ['~/' strrep(experiment_name,' ','_') '.1.png'];
result_image_location2 = ['~/' strrep(experiment_name,' ','_') '.1.png'];
archive_data_location = {[raw_data_dir '/' microarray_design '.' strrep(experiment_name,' ','_') '.SNP_data.mat'], ...
                         [raw_data_dir '/' microarray_design '.' strrep(experiment_name,' ','_') '.CGH_data.mat'], ...
                         [raw_data_dir '/' microarray_design '.' strrep(experiment_name,' ','_') '.datasetDetails.mat']};

new_fid = fopen('output2.txt','w');
