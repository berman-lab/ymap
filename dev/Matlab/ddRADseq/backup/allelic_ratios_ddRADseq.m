...
if (exist([projectDir 'SNP_' SNP_verString '.all.mat'],'file') == 0)
	fprintf('\nMAT file containing allelic_ratio data not found, regenerating.\n');
	C_datafile = [projectDir 'putative_SNPs_v4.txt'];
	P_datafile = [hapmapDir  'putative_SNPs_v4.txt'];
	C_data     = fopen(C_datafile, 'r');
	P_data     = fopen(P_datafile, 'r');

	% preallocate data vectors the length of each chromosome*2.
	...
	for chrID = 1:num_chrs
		C_chr_SNP_data_positions{chrID} = zeros(20000,1);
		C_chr_SNP_data_ratios   {chrID} = zeros(20000,1);
		C_chr_lines_analyzed(chrID)     = 0;
		P_chr_SNP_data_positions{chrID} = zeros(20000,1);
		P_chr_SNP_data_ratios   {chrID} = zeros(20000,1);
	...
	for chrID = 1:num_chrs
		if (chr_in_use(chrID) == 1)
			C_chr_SNP_data_ratios{   chrID}(P_chr_SNP_data_positions{chrID} == 0) = [];
			C_chr_SNP_data_positions{chrID}(P_chr_SNP_data_positions{chrID} == 0) = [];
	...
	save([projectDir 'SNP_' SNP_verString '.all.mat'],'C_chr_SNP_data_ratios','C_chr_SNP_data_positions','P_chr_SNP_data_ratios','P_chr_SNP_data_positions');
else
...
for chr = 1:num_chrs
	if (chr_in_use(chr) == 1)
	    ...
	    if (Linear_display == true)
			...
			% linear : show allelic ratio data.
			for datum = 1:length(C_chr_SNP_data_positions{chr})
				C_locus_position = C_chr_SNP_data_positions{chr}(datum);
				C_locus_ratio    = C_chr_SNP_data_ratios{   chr}(datum);
				plot(C_locus_position/bases_per_bin, C_locus_ratio*maxY         ,'Color',[1.0 0.0 0.0],'Marker','.','MarkerSize',2);
...
saveas(fig,        [projectDir 'fig.allelic_ratio-map.1.eps'], 'epsc');
saveas(fig,        [projectDir 'fig.allelic_ratio-map.1.png'], 'png');
set(Linear_fig,'PaperPosition',[0 0 8 0.62222222]*2);
saveas(Linear_fig, [projectDir 'fig.allelic_ratio-map.2.eps'], 'epsc');
saveas(Linear_fig, [projectDir 'fig.allelic_ratio-map.2.png'], 'png');
