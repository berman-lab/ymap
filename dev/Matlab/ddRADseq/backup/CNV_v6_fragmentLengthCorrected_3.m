function [] = CNV_v6_fragmentLengthCorrected_3(projectName,genome_data,referenceName,genome_ref,ploidyString,ploidyBaseString, ...
...
plot(X_length,Y_reads,'o', 'color', [0.0, 0.66667, 0.33333], 'MarkerSize', 1);
hold on;
...
plot(X_length,Y_reads_ref,'o', 'color', [0.0, 0.66667, 0.33333], 'MarkerSize', 1);
hold on;
plot(newX1, newY1,'color','k','linestyle','-', 'linewidth',2);
hold off;
axis normal;
...
for frag = 1:numFragments
	...
	plot(X_datum,Y_datum,'o', 'color', [0.0, 0.66667, 0.33333], 'MarkerSize',0.2);
    else
        plot(X_datum,Y_datum,'o', 'color', [1.0, 0.0, 0.0], 'MarkerSize',0.2);
    end;
    if (mod(frag,1000) == 0)
	fprintf('.');
    end;
...
fprintf('\n');
% trim datasets.
Y_reads              (X_length > 5000) = [];
Y_reads_corrected    (X_length > 5000) = [];
Y_reads_ref          (X_length > 5000) = [];
Y_reads_corrected_ref(X_length > 5000) = [];
X_repet              (X_length > 5000) = [];
X_length             (X_length > 5000) = [];
fprintf(['\tlength(X_length)              = ' num2str(length(X_length))              '\n']);
fprintf(['\tlength(Y_reads)               = ' num2str(length(Y_reads))               '\n']);
fprintf(['\tlength(Y_reads_ref)           = ' num2str(length(Y_reads_ref))           '\n']);
fprintf(['\tlength(Y_reads_corrected)     = ' num2str(length(Y_reads_corrected))     '\n']);
...
% end;
sh(5) = subplot(5,4,17);
plot(X_repet,Y_reads_corrected,'.', 'color', [0.0, 0.66667, 0.33333], 'MarkerSize', 1);
% hold on;
% plot(newX2,newY2,'color','k','linestyle','-', 'linewidth',2);
% hold off;
axis normal;
...
% end;
sh(6) = subplot(5,4,19);
plot(Y_reads_corrected,Y_reads_corrected_ref,'o', 'color', [0.0, 0.66667, 0.33333], 'MarkerSize', 1);
% hold on;
% plot(newX2,newY2,'color','k','linestyle','-', 'linewidth',2);
% hold off;
axis normal;
...
if (exist([workingDir 'matlab_dir/' referenceName '.corrected_CNV_' CNV_verString '.ploidy_' ploidyString '.mat'],'file') == 0)
    ...
    save([workingDir 'matlab_dir/' referenceName '.corrected_CNV_' CNV_verString '.ploidy_' ploidyString '.mat'],...
         'chr_CNVdata_RADseq');
else
    fprintf('\nRefrence CNV MAT file found, loading.\n');
    load([workingDir 'matlab_dir/' referenceName '.corrected_CNV_' CNV_verString '.ploidy_' ploidyString '.mat']);
end;
...
%% -----------------------------------------------------------------------------------------
% Output corrected CNV data to pileup-type file.    dddd
%-------------------------------------------------------------------------------------------
