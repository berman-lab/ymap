...
for chr = 1:num_chrs
	if (chr_in_use(chr) == 1)
		...
		% render the current fragment as image, for grabbing existing colors along lines.
		% 	imageMap = getframe(subPlotHandle);
		% fprintf(['\n\n@@@@\n@@@@ subfigure ' num2str(chr) ' size = ' num2str(size(imageMap)) '\n@@@@\n']);
			...
			for segment = 1:length(chrCopyNum{chr})
					...
					if (length(coordinate_phased) > 0)
						for i = 1:length(coordinate_phased)
							...
							if ((coordinate_phased(i) > chr_breaks{chr}(segment)*chr_length) && (coordinate_phased(i) <= chr_breaks{chr}(segment+1)*chr_length))
								...
								histAll(SnpLocusCount) = ratioData_phased(i);
								dataCount = dataCount+1;
							else
								histAll(SnpLocusCount) = 0;
							end;
						end;
					end;
					if (length(coordinate_unphased) > 0)
						for i = 1:length(coordinate_unphased)
							...
							if ((coordinate_unphased(i) > chr_breaks{chr}(segment)*chr_length) && (coordinate_unphased(i) <= chr_breaks{chr}(segment+1)*chr_length))
								...
								histAll(SnpLocusCount) = ratioData_unphased(i);
								dataCount = dataCount+1;
							else
								histAll(SnpLocusCount) = 0;
							end;
				...
				for region = mostLikelyGaussians
						...
						if (show_uncalibrated == true)
							...
							elseif (copynum == 7) %heptasomy
								...
								if (segment == 1)
									...
									set(gca,'XTickLabel',{'a', '', 'aaaaabb', '', '', 'aabbbbb', '' 'b'});
								end;
					...
					if (length(mostLikelyGaussians) == 1)
						...
						if (region_ == 1)
							area(         round(200-actual_cutoffs(region_)):200                                 , ...
							     smoothed(round(200-actual_cutoffs(region_)):200                                 ) ...
							     ,'FaceColor',color,'EdgeColor',color);
						elseif (region_ == length(mostLikelyGaussians))
							area(         1                                 :round(200-actual_cutoffs(region_-1)), ...
							     smoothed(1                                 :round(200-actual_cutoffs(region_-1))) ...
							     ,'FaceColor',color,'EdgeColor',color);
						else
							area(         round(200-actual_cutoffs(region_)):round(200-actual_cutoffs(region_-1)), ...
							     smoothed(round(200-actual_cutoffs(region_)):round(200-actual_cutoffs(region_-1))) ...
							    ,'FaceColor',color,'EdgeColor',color);
						end;
					end;
				end;
	    ...
	    if (Linear_display == true)
			...
			set(gca,'XTickLabel',[]);
			if (chr == 1)
				ylabel({project;'vs. (hapmap)';hamap}, 'Rotation', 0, 'HorizontalAlign', 'right', 'VerticalAlign', 'bottom','Interpreter','none','FontSize',10);
				set(gca,'YTick',[0 maxY/4 maxY/2 maxY/4*3 maxY]);
				set(gca,'YTickLabel',{'','','','',''});
				text(-50000/bases_per_bin, maxY/4,   '1','HorizontalAlignment','right','Fontsize',10);
				text(-50000/bases_per_bin, maxY/2,   '2','HorizontalAlignment','right','Fontsize',10);
