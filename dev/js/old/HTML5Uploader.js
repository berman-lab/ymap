...
(function ($, Handlebars) {
	...
	$.widget('filkor.html5Uploader', $.blueimp.fileupload, {
		options: {					autoUpload:			false,					// true = load on selection; false = load on upload button.
									...
									send:				function (e, data) {	// CALLBACK : invoked at the start of each file upload request.
									...
									done:				function (e, data) {	// CALLBACK : called upon completion of upload.
															var that     = $(this).data('filkor-html5Uploader'),
															...
															// Concluding script "conclusion_script" needs to be run here.   ddd
														//	formString = "<form action='"+conclusion_script+"' method='post'>";
														//	formString += "<input type='hidden' name='fileName' value='"+file.name+"'>";
														//	formString += "<input type='hidden' name='user'     value='"+user+"'>";
														//	formString += "<input type='hidden' name='genome'   value='"+genome+"'>";
														//	formString += "</form>";
														//	$(formString).submit();
													//		formString = "<form action='"+conclusion_script+"' method='post'><input type='hidden' name='fileName' value='"+file.name+"'></form>";
													//		$(formString).submit();
												//			$("<form action='"+conclusion_script+"' method='post'><input type='hidden' name='fileName' value='"+file.name+"'></form>").submit();
															$("<form action='"+conclusion_script+"' method='post'><input type='hidden' name='fileName' value='"+file.name+"'><input type='hidden' name='user' value='"+user+"'><input type='hidden' name='genome' value='"+genome+"'><input type='hidden' name='key' value='"+key+"'></form>").submit();
														},
