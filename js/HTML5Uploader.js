...
(function ($, Handlebars) {
	...
	$.widget('filkor.html5Uploader', $.blueimp.fileupload, {
		options: {
			autoUpload:		false,			// true = load on selection; false = load on upload button.
			...
			getNumberOfFiles:
				function(){
					// Function to return the current number of files.   (used by maxNumberOfFiles validation.)
					return this.filesContainer.children().length;
				},
			add:	function (e, data) {
					// CALLBACK : invoked when files are added to the fileupload widget.
					var $this     = $(this),
			...
			send:	function (e, data) {
					// CALLBACK : invoked at the start of each file upload request.
					...
					// CALLBACK : called upon completion of each file upload request.
					var that     = $(this).data('filkor-html5Uploader'),
			...
			stop:	function (e, data) {
					// CALLBACK : called upon conclusion of all uploads.
			...
			progress:
				function (e, data) {
					// CALLBACK : for updating the progress-bar during upload.
					if (data.context) {
			...
			progressall:
				function (e, data) {
					// CALLBACK : for updating the time and bitrate indications.
					var $this   = $(this),
			...
			getFileList:
				function (data) {
					// CALLBACK : retrieve list of files from the server response.
		...
		_initTemplates:
			function () {
				// Initialize & compile Handelbars template defined as id="template-upload".
		...
		_cancelHandler:
			function (e) {
				// CALLBACK : invoked when cancel button is pressed during upload.
