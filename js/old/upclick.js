...
function upclick(params)
{
    ...
    frame.onload_callback =
        function(){
            ...
            // Create form in page for data file loading.
            ...
            // ['action_params'] : Add hidden input element to form with server script parameters.
			...
			// ['maxsize'] : Add hidden input element to form with max file size.
			if (params['maxsize']){
			...
			// ['dataname'] : Add file input element to form, resulting in popup of a file selection window.
			input       = doc.createElement("input");
			input.name  = 'Filedata'; //params['dataname'];
			input.type  = 'file';
			input.size  = '1';
			input.runat = 'server';
            ...
            frame.onload_callback =
                function(){
                    ...
                    // Get frame window
                    if (frame.contentWindow){   // IE5.5+, Mozilla, NN7
                        ifwin = frame.contentWindow;
                    } else if (frame.contentDocument){   // NN6, Konqueror
    ...
    if (container.addEventListener){   // DOM2: FF, Chrome, Opera
        container.addEventListener('mousemove', onmouseout_callback, false);
	} else if (container.attachEvent){   // IE 5+
    ...
    if (element.addEventListener){   // DOM2: FF, Chrome, Opera
